...
def serve_static(path):
    logger.info(f"Looking for static file: {path}")
    # First check if the file exists in the static folder
    if os.path.exists(os.path.join(app.static_folder, path)):
        logger.info(f"Found static file: {path}")
        return send_from_directory(app.static_folder, path)
    else:
        # If not, return index.html for client-side routing
        logger.info(f"File not found, serving index.html instead for: {path}")
        return send_from_directory(app.static_folder, 'index.html')
